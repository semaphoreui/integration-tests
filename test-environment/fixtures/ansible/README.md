...
Он не изменяет систему и выводит только детерминированные безопасные маркеры. `variables.yml` проверяет обычные и секретные значения Variable Group через `no_log`; `survey-overrides.yml` проверяет survey values, env-target, template/task arguments и Ansible launch params. Секреты сравниваются по SHA-256 и не записываются в playbook или task output.
