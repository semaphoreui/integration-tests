...
`template-update.yml` verifies that updating a template changes its executed playbook and applies the saved `--check` argument. It asserts Ansible check mode and emits a deterministic marker without modifying the host.
