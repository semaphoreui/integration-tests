...
**Runtime baseline:** `semaphoreui/semaphore:v2.19.8`  
...
reproduced on `v2.19.8`. The affected source paths were compared with the current stable `v2.19.12`
tag on 2026-09-04. Unless stated otherwise, the relevant implementation is unchanged, so a runtime
recheck on `v2.19.12` is still required before claiming that release as independently reproduced.
...
|---|---|---:|---|---|
| BUG-001 | Project deletion succeeds while a task is still running | High | Projects / task execution | Confirmed on `v2.19.8`; affected stable code unchanged |
| BUG-002 | File inventory creation accepts a path outside its repository | High | Inventory API / path validation | Confirmed on `v2.19.8`; affected stable code unchanged |
...
| BUG-007 | Project restore accepts duplicate resource names | Medium | Project backup / restore | Confirmed on `v2.19.8`; off-by-one remains in `v2.19.12` and `develop` |
...
## Observation under monitoring — terminal task status can precede output persistence

This is intentionally not listed as a confirmed current defect. On a local `v2.19.8` environment,
terminal `success` or `error` occasionally became visible before final output and stages were fully
persisted. The issue reproduced repeatedly on SQLite and during local upgrade checks, but did not
reproduce in the Linux CI runs used for release validation.

Keep the existing diagnostic assertions and collect more current-stable evidence before filing it as
a standalone bug. See [v2.19.8-regression-report.md](v2.19.8-regression-report.md).
...
1. Move a temporary matrix lane to `v2.19.12` and rerun BUG-001, BUG-002, BUG-003 and BUG-007.
...
4. Test `develop` for BUG-004 and BUG-008 and convert both canaries to positive regression checks.
